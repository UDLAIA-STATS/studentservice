...
class TeamStatsPdfView(APIView):
    ...
    # Mensaje mostrado cuando no se puede confiar en el número de camiseta
    # ni en el id del jugador para identificarlo (camiseta 0, id nulo o 0).
    SHIRT_VALIDATION_MSG = "Validar con imagen de torso el número del jugador"
    ...
    def _aggregate_stats(self, player_rows):
        ...
        for row in player_rows:
            ...
            # Todas las filas de un mismo player_id deberían traer el mismo
            # shirt_number; nos quedamos con el primero que aparezca.
            if b["shirt_number"] is None:
                b["shirt_number"] = row.shirt_number
    ...
    def _needs_shirt_validation(self, player_id, shirt_number):
        """
        True cuando no se puede confiar en la identidad del jugador:
        camiseta 0, o id de jugador nulo o 0.
        """
        return shirt_number == 0 or not player_id

    def _player_display_name(self, jugador_obj, fallback_id, shirt_number=None):
        if self._needs_shirt_validation(fallback_id, shirt_number):
            return self.SHIRT_VALIDATION_MSG
        if jugador_obj:
    ...
    def _build_pdf(self, match_id, per_player, team_totals, team_image_paths,
        ...
        for row in per_player:
            ...
            name = self._player_display_name(jugador_obj, row["player_id"], row["shirt_number"])
            table_data.append([
                name,
        ...
        if not player_rows:
            ...
            for row in player_rows:
                jugador_obj = jugadores_map.get(row.player_id)
                needs_validation = self._needs_shirt_validation(row.player_id, row.shirt_number)
                name = self._player_display_name(jugador_obj, row.player_id, row.shirt_number)

                sub_parts = []
                # No mostramos el número de camiseta cuando el nombre ya está
                # marcado como "a validar": mostrar "#0" junto al aviso sería
                # contradictorio e induciría a error.
                if row.shirt_number and not needs_validation:
