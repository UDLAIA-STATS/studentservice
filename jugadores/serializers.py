...
class JugadorSerializer(serializers.ModelSerializer):
    ...
    def validate_idbanner(self, value):
        ...
        if self.instance:
            if (
                Jugadores.objects.exclude(pk=self.instance.pk)
                .filter(idbanner=value)
                .exists()
            ):
                raise serializers.ValidationError(
    ...
    def validate_posicionjugador(self, value):
        if value not in POSICIONES_VALIDAS:
            raise serializers.ValidationError("Debe seleccionar una posición válida.")
        return value
    ...
    def validate(self, attrs):
        ...
        if activo and numero:
            query = Jugadores.objects.filter(
                numerocamisetajugador=numero, jugadoractivo=True
            )
    ...
    def to_internal_value(self, data):
        ...
        if isinstance(imagen, str) and imagen.startswith("data:image"):
            try:
                ...
                raise serializers.ValidationError(
                    {"imagenjugador": "Formato Base64 inválido."}
                )
    ...
    def to_representation(self, instance):
        ...
        if instance.imagenjugador:
            rep["imagenjugador"] = (
                f"data:image/png;base64,{base64.b64encode(instance.imagenjugador).decode('utf-8')}"
            )
        else:
...
class JugadorUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        numero = attrs.get("numerocamisetajugador", self.instance.numerocamisetajugador)
        activo = attrs.get("jugadoractivo", self.instance.jugadoractivo)

        if activo and numero > 0:
            if (
                Jugadores.objects.exclude(pk=self.instance.pk)
                .filter(numerocamisetajugador=numero, jugadoractivo=True)
                .exists()
            ):
                raise serializers.ValidationError(
                    "El número de camiseta ya está asignado a otro jugador activo."
